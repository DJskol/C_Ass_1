...
char last_error_message[100] = "";
void RS232_StoreLastError(const char *fmt, ...);
...
int RS232_OpenComport(int comport_number, int baudrate, const char *mode, int flowctrl)
{
  ...
  if((comport_number>=RS232_PORTNR)||(comport_number<0))
  {
    ...
    RS232_StoreLastError("illegal comport number");
    return(1);
  }
  ...
  switch(baudrate)
  {
    ...
    RS232_StoreLastError("invalid baudrate\n");
                   return(1);
                   break;
  }
  ...
  if(strlen(mode) != 3)
  {
    ...
    RS232_StoreLastError("invalid mode \"%s\"\n", mode);
    return(1);
  }
  ...
  switch(mode[0])
  {
    ...
    RS232_StoreLastError("invalid number of data-bits '%c'\n", mode[0]);
              return(1);
              break;
  }
  ...
  switch(mode[1])
  {
    ...
    RS232_StoreLastError("invalid parity '%c'\n", mode[1]);
              return(1);
              break;
  }
  ...
  switch(mode[2])
  {
    ...
    RS232_StoreLastError("invalid number of stop bits '%c'\n", mode[2]);
              return(1);
              break;
  }
...
*/
  ...
  if(Cport[comport_number]==-1)
  {
    ...
    RS232_StoreLastError("unable to open comport ");
    return(1);
  }
  ...
  if(flock(Cport[comport_number], LOCK_EX | LOCK_NB) != 0)
  {
    ...
    RS232_StoreLastError("Another process has locked the comport.");
    return(1);
  }
  ...
  if(err==-1)
  {
    ...
    RS232_StoreLastError("unable to read portsettings ");
    return(1);
  }
  ...
  if(err==-1)
  {
    ...
    RS232_StoreLastError("unable to adjust portsettings ");
    return(1);
  }
...
/* https://man7.org/linux/man-pages/man4/tty_ioctl.4.html */
  ...
  if(ioctl(Cport[comport_number], TIOCMGET, &status) == -1)
  {
    ...
    RS232_StoreLastError("unable to get portstatus");
    return(1);
  }
  ...
  if(ioctl(Cport[comport_number], TIOCMSET, &status) == -1)
  {
    ...
    RS232_StoreLastError("unable to set portstatus");
    return(1);
  }
...
void RS232_enableBREAK(int comport_number)
{
  if(ioctl(Cport[comport_number], TIOCSBRK, NULL) == -1)  /* Turn break on, that is, start sending zero bits. */
  {
    ...
    RS232_StoreLastError("unable to turn break on");
  }
}
...
void RS232_disableBREAK(int comport_number)
{
  if(ioctl(Cport[comport_number], TIOCCBRK, NULL) == -1)  /* Turn break off, that is, stop sending zero bits. */
  {
    ...
    RS232_StoreLastError("unable to turn break off");
  }
}
...
bool RS232_IsConnected(int com_port) {
    int status;
    return ioctl(Cport[com_port], TIOCMGET, &status) != -1;
}
...
*/
  ...
  if(!BuildCommDCBA(mode_str, &port_settings))
  {
    ...
    RS232_StoreLastError("unable to set comport dcb settings\n");
  ...
  if(!SetCommState(Cport[comport_number], &port_settings))
  {
    ...
    RS232_StoreLastError("unable to set comport cfg settings\n");
    CloseHandle(Cport[comport_number]);
    return(1);
  }
  ...
  if(!SetCommTimeouts(Cport[comport_number], &Cptimeouts))
  {
    ...
    RS232_StoreLastError("unable to set comport time-out settings\n");
    CloseHandle(Cport[comport_number]);
    return(1);
  }
...
bool RS232_IsConnected(int com_port) {
    DWORD status;
    return GetCommModemStatus((HANDLE)Cport[com_port], &status);
    // int error_code = GetLastError();
    // DCB dcb;
    // success = GetCommState((HANDLE)Cport[com_port], &dcb);
    // DWORD error;
    // COMSTAT stat;
    // bool ret_error = ClearCommError(Cport[com_port], &error, &stat);
}
...
void RS232_StoreLastError(const char *fmt, ...) {
  va_list args;
  va_start(args, fmt);
  vsprintf(last_error_message, fmt, args);
  va_end(args);
}

void RS232_GetLastErrorMsg(char *buffer, bool clear_error) {
  strcpy(buffer, last_error_message);
  if (clear_error) {
    strcpy(last_error_message, "");
  }
}
